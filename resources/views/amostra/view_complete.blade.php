               ...
               <div class="card-body">
                   ...
                   @foreach ($alteracoes as $alteracao)
                       @php
                           ...
                           ->leftJoin('amostra_alteracao_parametro', 'parametro.id', '=', 'amostra_alteracao_parametro.fk_parametro' )
                           ->leftJoin('categoria', 'categoria.id', '=', 'nota_parametro' )
                           ->select('parametro.*', 'categoria.nota')
                           ->where('fk_alteracao', '=', $alteracao->id)
                           ->where('fk_amostra', '=', $amostra->id)
                           ->get();
                       @endphp
                       <table>
                           <tbody>
                               <tr>
                                   ...
                                   <td><b>@if(isset($alteracao->nota)){{$alteracao->nota}}@else Não Avaliado @endif </b></td>
                               </tr>
                               @foreach ($parametros as $parametro)
                                   <tr>
                                       <td>{{$parametro->descricao}}:</td>
                                       <td>@if(isset($parametro->nota)){{$parametro->nota}}@else Não Avaliado @endif</td>
                                   </tr>
                               @endforeach
                           </tbody>
                       </table>
